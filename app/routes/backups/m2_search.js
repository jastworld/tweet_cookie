		...
		if (limit == 0 || (username != null && username == currUser) && following)
			...
			if (timestamp != null) {
				if (username == null && q == null) {
					const query = 'SELECT JSON * FROM item WHERE timestamp <= ? LIMIT ? ALLOW FILTERING';
					const params = [timestamp, limit];
					executeQuery(query, params);
				} else if (username != null && q == null) {
					const query = 'SELECT JSON * FROM item WHERE timestamp <= ? AND username = ? LIMIT ? ALLOW FILTERING';
					const params = [timestamp, username, limit];	
					executeQuery(query, params);
				} else if (username == null && q != null) {
					const query = 'SELECT JSON * FROM item WHERE timestamp <= ? AND content LIKE ? LIMIT ? ALLOW FILTERING';
					const params = [timestamp, q, limit];
					executeQuery(query, params);
				} else {
					const query = 'SELECT JSON * FROM item WHERE timestamp <= ? AND username = ? AND content LIKE ? LIMIT ? ALLOW FILTERING';
					const params = [timestamp, username, q, limit];
					executeQuery(query, params);
				};
			} else {
				if (username == null && q == null) {
                    const query = 'SELECT JSON * FROM item LIMIT ?';
                    const params = [timestamp, limit];
                    executeQuery(query, params);
                } else if (username != null && q == null) {
                    const query = 'SELECT JSON * FROM item WHERE username = ? LIMIT ?';
                    const params = [timestamp, username, limit]; 
                    executeQuery(query, params);
                } else if (username == null && q != null) {
                    const query = 'SELECT JSON * FROM item WHERE content LIKE ? LIMIT ?';
                    const params = [timestamp, q, limit];
                    executeQuery(query, params);
                } else {
                    const query = 'SELECT JSON * FROM item WHERE username = ? AND content LIKE ? LIMIT ?';
			...
			User.findOne({ username: currUser },(err,result)=>{	
				if(err) {
					...
					if (timestamp != null) { 
						if (username == null && q == null) {
							const query = 'SELECT JSON * FROM item WHERE timestamp <= ? AND username IN ? LIMIT ? ALLOW FILTERING';
							...
							const query = 'SELECT JSON * FROM item WHERE timestamp <= ? AND content LIKE ? ALLOW FILTERING';
							const params = [timestamp, q];
							executeManQuery(query, params, limit, users);
						} else {
							const query = 'SELECT JSON * FROM item WHERE timestamp <= ? AND username = ? AND content LIKE ? ALLOW FILTERING';
							const params = [timestamp, username, q];
							executeManQuery(query, params, limit, users);	
						};
					} else {
                        if (username == null && q == null) {
                            const query = 'SELECT JSON * FROM item WHERE username IN ? LIMIT ?';
                            ...
                            const query = 'SELECT JSON * FROM item WHERE content LIKE ?';
                            const params = [timestamp, q];
                            executeManQuery(query, params, limit, users);
                        } else {
                            const query = 'SELECT JSON * FROM item WHERE username = ? AND content LIKE ?';
